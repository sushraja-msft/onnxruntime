...
Status ConvOpBuilder::ProcessConv2D3DInputs(QnnModelWrapper& qnn_model_wrapper,
  ...
  //
  {
    ...
    if (input_info.is_initializer) {
      ...
      if (conv_type == OnnxConvType::kConv) {
        ORT_RETURN_IF_ERROR(
            utils::TransposeFromNchwToHwcn(qnn_model_wrapper, *input_info.initializer_tensor, unpacked_tensor, is_3d));
      } else if (conv_type == OnnxConvType::kConvTranspose) {
        ORT_RETURN_IF_ERROR(
            utils::TransposeFromCnhwToHwcn(qnn_model_wrapper, *input_info.initializer_tensor, unpacked_tensor, is_3d));
      } else {
        return ORT_MAKE_STATUS(ONNXRUNTIME, FAIL,
                               "QNN EP: Unexpected convolution op type: ", node_unit.OpType().c_str());
      }
