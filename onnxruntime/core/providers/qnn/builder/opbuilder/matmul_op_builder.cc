...
Status MatMulOpBuilder::ProcessInputs(QnnModelWrapper& qnn_model_wrapper, const NodeUnit& node_unit,
  ...
  ORT_RETURN_IF_NOT(qnn_model_wrapper.GetOnnxShape(inputs[0].node_arg, input_0_shape), "Cannot get shape");
  ...
  // Use FullyConnected for 2D initializer as 2nd input, need to do transpose.
  if (is_initializer_input_1 && input_1_shape.size() == 2) {
    QnnQuantParamsWrapper quantize_param;
    ORT_RETURN_IF_ERROR(quantize_param.Init(qnn_model_wrapper, inputs[1]));
    bool is_quantized_tensor = inputs[1].quant_param.has_value();
    Qnn_DataType_t qnn_data_type = QNN_DATATYPE_FLOAT_32;
    ORT_RETURN_IF_ERROR(utils::GetQnnDataType(is_quantized_tensor, inputs[1].node_arg.TypeAsProto(), qnn_data_type));
    ...
    ORT_RETURN_IF_ERROR(
        utils::TwoDimensionTranspose(qnn_model_wrapper, input_1_shape, *input_1_tensor, unpacked_tensor));
  ...
  if (is_initializer_input_1 && input_1_shape.size() == 2) {
    ...
    std::vector<uint32_t> op_output_shape =
        need_reshape ? std::vector<uint32_t>({std::accumulate(org_output_shape.begin(), org_output_shape.end() - 1,
                     ...
                     : org_output_shape;
    QnnTensorWrapper output_tensor_wrapper(op_output_name, QNN_TENSOR_TYPE_NATIVE, output_info.qnn_data_type,
    ...
    if (need_reshape) {
      ...
      ORT_RETURN_IF_ERROR(qnn_model_wrapper.AddReshapeNode(
          ...
          output_info.quant_param.Copy(), do_op_validation, false, is_graph_output));
    }
  } else {
