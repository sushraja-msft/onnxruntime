...
bool GetQSalarScaleZp(const GraphViewer& graph_viewer, const Node& q_node, float& scale, int32_t& zp, int32_t& data_type) {
  ...
  const ONNX_NAMESPACE::TensorProto* scale_tensor_proto = graph_viewer.GetConstantInitializer(q_input_defs[1]->Name(), true);
  ...
  const ONNX_NAMESPACE::TensorProto* zp_tensor_proto = graph_viewer.GetConstantInitializer(q_input_defs[2]->Name(), true);
  if (!zp_tensor_proto) {
    return false;
  }
