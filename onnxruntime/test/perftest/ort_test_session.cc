...
OnnxRuntimeTestSession::OnnxRuntimeTestSession(Ort::Env& env, std::random_device& rd,
  ...
  if (provider_name_ == onnxruntime::kDnnlExecutionProvider) {
    ...
    while (ss >> token) {
      ...
      } else if (key == "enable_htp_fp16_precision"|| key == "enable_qnn_graph_dump" || key == "offload_graph_io_quantization") {
...
'htp_arch', 'device_id', 'enable_htp_fp16_precision', 'enable_qnn_graph_dump', 'qnn_graph_dump_dir', 'offload_graph_io_quantization'])");
      }
